  ...
  return <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-200">
        <div className="flex items-center gap-1.5">
          <FlipHorizontalIcon className="w-4 h-4 text-slate-500" />
        ...
        <button onClick={() => updateParam('flipHorizontal', !params.flipHorizontal)} className={`relative w-11 h-6 rounded-full transition-colors ${params.flipHorizontal ? 'bg-blue-600' : 'bg-slate-300'}`}>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <button onClick={() => setShowBlendModeDropdown(!showBlendModeDropdown)} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-sm text-slate-700 transition-colors flex items-center justify-between hover:bg-slate-100 cursor-pointer shadow-sm">
            <span>{BLEND_MODE_LABELS[params.blendMode]}</span>
            <ChevronDownIcon className={`w-4 h-4 text-slate-400 transition-transform ${showBlendModeDropdown ? 'rotate-180' : ''}`} />
          </button>
          ...
          {showBlendModeDropdown && <>
            ...
            zIndex: 9997
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 shadow-xl overflow-hidden max-h-64 overflow-y-auto" style={{
            zIndex: 9999
          }}>
            ...
            }} className={`w-full text-left px-3 py-2 hover:bg-slate-50 transition-colors text-sm ${params.blendMode === mode ? 'text-blue-700 bg-blue-50 font-medium' : 'text-slate-600'}`}>
      ...
      <div className="space-y-1.5">
        ...
        <input type="range" min={-300} max={300} step={1} value={params.frameOffset} onChange={e => updateParam('frameOffset', parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-200 appearance-none cursor-pointer
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => updateParam('stackingOrder', 0)} className={`px-3 py-2 text-xs font-medium transition-colors cursor-pointer border ${params.stackingOrder === 0 ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
            Source on Top
          </button>
          <button onClick={() => updateParam('stackingOrder', 1)} className={`px-3 py-2 text-xs font-medium transition-colors cursor-pointer border ${params.stackingOrder === 1 ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
            Duplicate on Top
          </button>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <input type="range" min={0} max={100} step={1} value={params.opacity} onChange={e => updateParam('opacity', parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-200 appearance-none cursor-pointer
            [&::-webkit-slider-thumb]:appearance-none
            [&::-webkit-slider-thumb]:w-3
            [&::-webkit-slider-thumb]:h-3
            [&::-webkit-slider-thumb]:rounded-none
